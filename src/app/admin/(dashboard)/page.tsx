...
export default async function AdminDashboard({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  let query = admin.from("attempts").select("id,student_name,class_name,score,max_score,percentage,correct_count,total_questions,duration_seconds,submitted_at,quizzes(title)", { count: "exact" }).eq("status", "submitted");
