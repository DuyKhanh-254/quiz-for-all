...
export function QuestionCard(props: Props) {
  ...
  return <article id={`question-${question.id}`} className={`card scroll-mt-24 p-5 sm:p-7 ${isTestFive ? "test5-question-card relative overflow-hidden !border-[#94c11f] !bg-[#fffef8] sm:!p-8" : ""}`}>
    <header className={`mb-5 flex items-start justify-between gap-4 ${isTestFive ? "text-center sm:justify-center" : ""}`}><div className={isTestFive ? "w-full" : ""}><p className="text-sm font-extrabold uppercase tracking-wider text-[#617a94]">Câu {number}</p><h3 className={`${isTestFive ? "mx-auto mt-3 max-w-4xl text-xl sm:text-2xl" : "mt-1 text-xl"} font-extrabold leading-snug`}>{question.prompt}</h3></div><Status isCorrect={isCorrect} /></header>
