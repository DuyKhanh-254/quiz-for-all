param(
    [string]$ContentName = "test-vocab"
)

$supabaseUrl = "https://syghsisooccdvpvshgvm.supabase.co"
$serviceKey = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InN5Z2hzaXNvb2NjZHZwdnNoZ3ZtIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc4Njg3ODM5NSwiZXhwIjoyMTAyNDU0Mzk1fQ.kVg4wYbtpw2T5jqRbNL3g-zDDqeNZsa1WAs1gyFAzfw"

$headers = @{
    "apikey" = $serviceKey
    "Authorization" = "Bearer $serviceKey"
    "Content-Type" = "application/json"
    "Prefer" = "resolution=merge-duplicates"
}

$publicPath = Join-Path $PSScriptRoot "..\content\$ContentName.public.json"
$privatePath = Join-Path $PSScriptRoot "..\content\$ContentName.private.json"

if (-not (Test-Path $publicPath)) { throw "Public content file not found: $publicPath" }
if (-not (Test-Path $privatePath)) { throw "Private content file not found: $privatePath" }

$publicContent = Get-Content $publicPath -Raw | ConvertFrom-Json
$privateAnswers = Get-Content $privatePath -Raw | ConvertFrom-Json

Write-Host "Importing Quiz: $($publicContent.quiz.title)..."

# 1. Upsert Quiz
$quizBody = $publicContent.quiz | ConvertTo-Json -Depth 5
try { Invoke-RestMethod -Uri "$supabaseUrl/rest/v1/quizzes?on_conflict=id" -Method Post -Headers $headers -Body $quizBody } catch {}
Write-Host "Quiz inserted."

# 2. Upsert Sections & Questions
foreach ($section in $publicContent.sections) {
    $sectionRow = @{
        id = $section.id
        quiz_id = $publicContent.quiz.id
        title = $section.title
        instruction = $section.instruction
        section_type = $section.section_type
        position = $section.position
    }
    $sectionBody = $sectionRow | ConvertTo-Json -Depth 5
    try { Invoke-RestMethod -Uri "$supabaseUrl/rest/v1/quiz_sections?on_conflict=id" -Method Post -Headers $headers -Body $sectionBody } catch {}

    foreach ($question in $section.questions) {
        $qRow = @{
            id = $question.id
            quiz_id = $publicContent.quiz.id
            section_id = $section.id
            position = $question.position
            question_type = $question.question_type
            prompt = $question.prompt
            points = $question.points
            metadata = if ($question.metadata) { $question.metadata } else { @{} }
        }
        $qBody = $qRow | ConvertTo-Json -Depth 5
        try { Invoke-RestMethod -Uri "$supabaseUrl/rest/v1/questions?on_conflict=id" -Method Post -Headers $headers -Body $qBody } catch {}

        # Delete existing options for this question
        $deleteHeaders = @{ "apikey" = $serviceKey; "Authorization" = "Bearer $serviceKey" }
        try { Invoke-RestMethod -Uri "$supabaseUrl/rest/v1/question_options?question_id=eq.$($question.id)" -Method Delete -Headers $deleteHeaders } catch {}

        # Insert Options
        if ($question.options -and $question.options.Count -gt 0) {
            $optionRows = @()
            $optPos = 1
            foreach ($opt in $question.options) {
                $optionRows += @{
                    question_id = $question.id
                    option_key = $opt[0]
                    option_text = $opt[1]
                    position = $optPos
                }
                $optPos++
            }
            $optBody = $optionRows | ConvertTo-Json -Depth 5
            try { Invoke-RestMethod -Uri "$supabaseUrl/rest/v1/question_options" -Method Post -Headers $headers -Body $optBody } catch {}
        }

        # Answer Key
        $qId = $question.id
        $ans = $privateAnswers.$qId
        if ($ans) {
            $keyRow = @{
                question_id = $question.id
                answer = $ans
            }
            $keyBody = $keyRow | ConvertTo-Json -Depth 5
            try { Invoke-RestMethod -Uri "$supabaseUrl/rest/v1/answer_keys?on_conflict=question_id" -Method Post -Headers $headers -Body $keyBody } catch {}
        }
    }
}

Write-Host "Successfully imported $($publicContent.quiz.title) with $($publicContent.sections.Count) sections!"
